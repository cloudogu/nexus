DOGU_V3_ARTIFACT_ID?=$(ARTIFACT_ID)
DOGU_V3_BUILD_VERSION := $(shell date +%s)
DOGU_V3_DEV_VERSION?=${VERSION}-dev.${DOGU_V3_BUILD_VERSION}
...
HELM_RELEASE_TGZ=${HELM_TARGET_DIR}/${DOGU_V3_ARTIFACT_ID}-${VERSION}.tgz
HELM_DEV_RELEASE_TGZ=${HELM_TARGET_DIR}/${DOGU_V3_ARTIFACT_ID}-${DOGU_V3_DEV_VERSION}.tgz
...
# This can be used by components/API v3 dogus with own images to build and push to the dev registry.
# Users should override this variable with the `image-import` target.
IMAGE_IMPORT_TARGET?=
...
helm-init-chart: ${BINARY_HELM} ## Creates a Chart.yaml-template with zero values
	...
	@${BINARY_HELM} create ${HELM_SOURCE_DIR}/tmp/${DOGU_V3_ARTIFACT_ID}
	@cp ${HELM_SOURCE_DIR}/tmp/${DOGU_V3_ARTIFACT_ID}/Chart.yaml ${HELM_SOURCE_DIR}/
...
${HELM_TARGET_DIR}/Chart.yaml: $(K8S_RESOURCE_TEMP_FOLDER) validate-chart ${HELM_PRE_GENERATE_TARGETS} copy-helm-files
	...
	@if [[ ${STAGE} == "development" ]]; then \
  	  sed -i 's/appVersion: "0.0.0-replaceme"/appVersion: '$(DOGU_V3_DEV_VERSION)'/' ${HELM_TARGET_DIR}/Chart.yaml; \
  	  sed -i 's/version: 0.0.0-replaceme/version:  '$(DOGU_V3_DEV_VERSION)'/' ${HELM_TARGET_DIR}/Chart.yaml; \
  	else \
...
helm-apply: ${BINARY_HELM} check-k8s-namespace-env-var ${IMAGE_IMPORT_TARGET} helm-generate ${HELM_PRE_APPLY_TARGETS} ## Generates and installs the Helm chart.
	...
	@${BINARY_HELM} --kube-context="${KUBE_CONTEXT_NAME}" upgrade -i ${DOGU_V3_ARTIFACT_ID} ${HELM_TARGET_DIR} ${BINARY_HELM_ADDITIONAL_UPGR_ARGS} --namespace ${NAMESPACE}

.PHONY: helm-delete
helm-delete: ${BINARY_HELM} check-k8s-namespace-env-var ## Uninstalls the current Helm chart.
	@echo "Uninstall helm chart"
	@${BINARY_HELM} --kube-context="${KUBE_CONTEXT_NAME}" uninstall ${DOGU_V3_ARTIFACT_ID} --namespace=${NAMESPACE} ${BINARY_HELM_ADDITIONAL_UNINST_ARGS} || true
...
helm-delete-existing-tgz: ## Remove an existing Helm package from the target directory.
	...
	@rm -f ${HELM_TARGET_DIR}/${DOGU_V3_ARTIFACT_ID}-*.tgz
